...
export async function POST(request: Request) {
  const { boardId, origin, revealAt } = (await request.json()) as {
    boardId: string;
    origin: string;
  ...
  // カウントダウン公開を選んだ場合、未来の日時であることをサーバー側でも確認する
  const hasCountdown = !!revealAt && new Date(revealAt).getTime() > Date.now();
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    metadata: {
      boardId: board.id,
      tier,
      revealAt: hasCountdown ? new Date(revealAt!).toISOString() : "",
    },
